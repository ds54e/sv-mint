        ...
        if scopes and class_depth == 0:
            for match in FUNCTION_RE.finditer(line):
                ...
                if " automatic" in segment or " static" in segment:
                    continue
                out.append({
                    "rule_id": "require_function_scope",
                    "severity": "warning",
...
def _check_randomize(text):
    ...
    out.extend(_randomize_matches(
        text,
        RANDOMIZE_WITH_RE,
        rule_id="dv_rand_macro_with_required",
        message="use DV_CHECK_*_WITH macros when randomizing with constraints",
    ))
    return out


def _randomize_matches(text, pattern, rule_id="dv_rand_macro_required", message="use DV_CHECK_* randomization macros instead of direct randomize()"):
    out = []
    ...
    for match in LOG_CALL_RE.finditer(text):
        ...
        if len(args) >= 3:
            ...
            if verb not in ALLOWED_VERBOSITY:
                out.append({
                    "rule_id": "require_log_allowed_verbosity",
                    "severity": "warning",
                    "message": "uvm report macros must use UVM_LOW/MEDIUM/HIGH/DEBUG verbosity",
                    "location": loc(text, match.start()),
                })
    for pattern, rule_id, message in (
        (UVM_WARNING_RE, "dv.log.no_uvm_warning", "use uvm_error or uvm_fatal instead of uvm_warning"),
        (UVM_REPORT_RE, "dv.log.no_uvm_report_api", "use uvm_{info,error,fatal} macros instead of uvm_report_*"),
        (DISPLAY_RE, "dv.log.no_display", "use uvm_* macros instead of $display"),
    ):
        for match in pattern.finditer(text):
            out.append({
                "rule_id": rule_id,
...
def _check_dpi(text):
    ...
    for match in IMPORT_RE.finditer(text):
        ...
        if not name.startswith("c_dpi_"):
            out.append({
                "rule_id": "require_dpi_import_prefix",
                "severity": "warning",
    ...
    for match in EXPORT_RE.finditer(text):
        ...
        if not name.startswith("sv_dpi_"):
            out.append({
                "rule_id": "require_dpi_export_prefix",
                "severity": "warning",
...
def _check_macros(text, path):
    ...
    for name, index in defs:
        ...
        if not macros_file and guarded:
            out.append({
                "rule_id": "forbid_macro_local_guard",
                "severity": "warning",
    ...
    for name in dv_macros:
        ...
        pos = text.find(name)
        out.append({
            "rule_id": "require_macro_prefix_header",
            "severity": "warning",
...
def _check_scoreboard(text):
    ...
    if not DV_EOT_RE.search(text):
        for match in SCOREBOARD_CLASS_RE.finditer(text):
            out.append({
                "rule_id": "require_scoreboard_eot",
                "severity": "warning",
...
def _check_comparison_macros(text):
    ...
    for match in IF_COMPARE_RE.finditer(text):
        ...
        if not UVM_REPORT_CALL_RE.search(window):
            continue
        out.append({
            "rule_id": "require_dv_check_macro",
            "severity": "warning",
...
def _check_module_macro_prefix(text):
    ...
    for start, end, name in _module_ranges(text):
        ...
        for match in re.finditer(r"`define\s+([A-Za-z_]\w*)", block):
            ...
            out.append({
                "rule_id": "require_macro_module_prefix",
                "severity": "warning",
