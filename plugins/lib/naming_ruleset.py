...
def _check_clock_reset(name, loc):
    ...
    if name.startswith("rst") and not (name.endswith("_n") or name.endswith("_ni") or name.endswith("_no") or name.endswith("_nio")):
        issues.append({
            "rule_id": "require_reset_active_low_suffix",
            "severity": "warning",
    ...
    for port in ports:
        ...
        if name.startswith("clk"):
            if clk_seen and rst_phase and loc:
                issues.append({
                    "rule_id": "require_clock_port_order",
                    "severity": "warning",
            ...
            if not clk_seen and loc:
                issues.append({
                    "rule_id": "require_reset_after_clock",
                    "severity": "warning",
        ...
        for name, port in names.items():
            if name.endswith("_p"):
                ...
                if twin not in names:
                    issues.append({
                        "rule_id": "require_differential_pair",
                        "severity": "warning",
    ...
    for name in names:
        ...
        if re_match:
            ...
            if prev not in names:
                issues.append({
                    "rule_id": "require_pipeline_sequence",
                    "severity": "warning",
...
def _check_parameter_naming(decls):
    ...
    for decl in decls:
        ...
        if not (UPPER_CAMEL.match(name) or ALL_CAPS.match(name)):
            issues.append({
                "rule_id": "require_param_uppercase",
                "severity": "warning",
...
def _check_port_direction_suffixes(ports):
    ...
    for port in ports:
        ...
        exp = " or ".join(allowed)
        issues.append({
            "rule_id": "require_port_direction_suffix",
            "severity": "warning",
