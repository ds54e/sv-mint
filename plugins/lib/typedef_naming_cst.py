...
def _check_enum(snippet, base, line_starts):
    ...
    for m in TYPEDEF_ENUM_RE.finditer(snippet):
        ...
        if not name.endswith("_e") or not LOWER_SNAKE.match(name):
            out.append(_make("typedef_enum_name_lower_snake_e", f"enum types should use lower_snake_case and end with _e: {name}", name_loc))
        body = m.group("body")
        ...
        for member, rel in _enum_entries(body):
            ...
            if not (UPPER_CAMEL.match(member) or ALL_CAPS.match(member)):
                out.append(_make("typedef_enum_value_upper", f"enum values should use UpperCamelCase or ALL_CAPS: {member}", loc))
    return out
...
def _check_typedef(snippet, base, line_starts):
    ...
    for m in TYPEDEF_RE.finditer(snippet):
        ...
        if not name.endswith("_t") or not LOWER_SNAKE.match(name):
            out.append(_make("typedef_type_name_lower_snake_t", f"typedef names should use lower_snake_case and end with _t: {name}", name_loc))
    return out
