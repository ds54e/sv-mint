...
fn parameter_missing_type_links_to_implicit_node() {
    let ir = load_ir("fixtures/parameter_has_type_missing_type_bad.sv");
...
fn parameter_with_type_has_explicit_type_field() {
    let ir = load_ir("fixtures/parameter_has_type_good.sv");
...
fn function_ports_expose_explicit_types() {
    let ir = load_ir("fixtures/functions_have_explicit_types_good.sv");
...
fn function_missing_types_mark_ports_and_return() {
    let ir = load_ir("fixtures/functions_have_explicit_types_bad.sv");
    let func = find_node(&ir, "FunctionDeclaration");
    let ret = field_id(func, "return_type");
    assert!(is_implicit_type(&ir, ret, None));
    let ports = func
...
fn always_events_track_or_separators() {
    let ir = load_ir("fixtures/sensitivity_list_uses_commas_bad.sv");
...
fn case_flags_indicate_unique_without_default() {
    let ir = load_ir("fixtures/case_has_default_branch_good.sv");
...
fn instance_connections_mark_positional_ports() {
    let ir = load_ir("fixtures/instances_use_named_ports_bad.sv");
