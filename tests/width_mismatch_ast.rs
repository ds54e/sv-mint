...
mod width_mismatch_ast_e2e {
    ...
    #[test]
    fn detects_mismatch_in_blocking_and_continuous() {
        if std::process::Command::new("py").arg("--version").output().is_err() {
            return;
        }
        let base = tmp("svmint_width_ast");
        fs::create_dir_all(&base).unwrap();
        let plugin_path = base.join("rules.py");
        fs::write(&plugin_path, include_str!("width_mismatch_ast_rules_inline.py")).unwrap();
