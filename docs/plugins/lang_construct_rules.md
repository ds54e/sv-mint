...
- **Trigger**: Checks `always_ff` sensitivity lists for the literal substring `negedge`; if absent, the rule fires (posedge or renamed resets still warn).
- **Message**: `` always_ff should include asynchronous reset (negedge rst_n) ``
- **Remediation**: Add `or negedge rst_ni` (or update the plugin if a different reset style is required) so the sensitivity list contains `negedge`.
