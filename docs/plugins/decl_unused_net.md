...
- **Additional Tips**: Only comments on the declaration line are considered; multi-line blocks or preceding comments do not suppress the warning. When mass-migrating generated RTL, disable the rule via its `[[rule]]` entry so real leftovers remain visible.
