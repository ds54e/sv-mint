...
"#,
    ...
    fn derives_scripts_without_plugin_root() {
        ...
        let script_path = plugins.join("require_dpi_import_prefix.raw.py");
...
"#,
        ...
        assert!(script.ends_with("require_dpi_import_prefix.raw.py"));
    }
...
"#,
        ...
        let resolved = super::plugin_search_paths(&cfg, "require_dpi_import_prefix.raw.py");
...
"#,
        ...
        match err {
            ConfigError::InvalidValue { detail } => {
                assert!(detail.contains("require_dpi_import_prefix"));
                assert!(detail.contains("searched"));
                let expected = tmp_dir.path().join("require_dpi_import_prefix.raw.py");
    ...
    fn resolves_script_under_relative_plugin_root() {
        ...
        let script_path = root.join("require_dpi_import_prefix.raw.py");
        fs::write(&script_path, "print('ok')").expect("script file");
        let mut cfg = load(
            r#"
[plugin]
...
"#,
        ...
        normalize_rule_scripts(&mut cfg, tmp_dir.path()).expect("normalize");
        infer_rule_stages(&mut cfg.rule).expect("stage");
        let paths = plugin_search_paths(&cfg, "require_dpi_import_prefix.raw.py");
        assert_eq!(paths[0], root.join("require_dpi_import_prefix.raw.py"));
    ...
    fn resolves_script_from_search_paths_and_root_prefers_root() {
        ...
        let in_root = root.join("require_dpi_import_prefix.raw.py");
        let in_extra = extra.join("require_dpi_import_prefix.raw.py");
